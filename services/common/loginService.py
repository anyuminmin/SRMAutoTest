...
log = Logger("LoginService").logger
...
class LoginService(BaseService):
	...
	def srm_buyer_login(self, paramDict):
	...
	def srm_buyer_choose_shop(self, paramDict):
	...
	def srm_supplier_login(self, paramDict):
		# loginName, password
	...
	def srm_supplier_choose_shop(self, paramDict):
		# securityToken, loginName, password, shopId
	...
	def srm_buyer_login_get_token(self):
		...
		try:
			data = self.srm_buyer_login({"accountNo": username, "password": des_ecb_encrypt(password)})["data"]
			if not data['securityToken']:
				...
				token = self.srm_buyer_choose_shop(paramDict)["data"]["token"]
	...
	def srm_supplier_login_get_token(self):
		...
		try:
			data = self.srm_supplier_login({"accountNo": username, "password": des_ecb_encrypt(password)})["data"]
			if not data['securityToken']:
				token = data['token']
			else:
				paramDict = {"loginName": username, "password": des_ecb_encrypt(password), "securityToken": data['securityToken'], "shopId": shop_id}
				token = self.srm_supplier_choose_shop(paramDict)["data"]["token"]
			with open(token_path, 'r', encoding='utf-8')as f:
				ele_info = f.read()
				yaml_dict = yaml.safe_load(ele_info)
			if yaml_dict is None:
	...
	def srm_buyer_query_scm_person(self):
		"""查询人员部门接口"""
	...
	def srm_buyer_query_shop_by_token(self):
	...
	def srm_buyer_query_user_permission_by_dept(self):
		"""获取权限接口"""
		rspDict = self.srm_buyer_query_scm_person()
...
if __name__ == '__main__':
	...
	userService.srm_buyer_login_get_token()
