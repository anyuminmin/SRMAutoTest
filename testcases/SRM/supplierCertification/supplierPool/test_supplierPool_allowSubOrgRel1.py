...
# 新增供应商库,允许下级引用：1所有，且维护或引用部分公司
def test_save_supplier_pool(json_headers):
	_data = replaceParams("MES_SAVE_SUPPLIER_POOL")
	_data['relOrganizationList'] = []  # 当为所有时，其入参引用公司为[]
	_data['allowSubOrgRel'] = 1
...
def test_select_save_supplier_pool(json_headers):
	response = requests.post(
	...
	assert response.json()['success']is True, response.json()['message']
	assert response.json()['data']['list'][0]['allowSubOrgRel'] == 1 , "列表允许公司下级引用方式与请求不一致"
...
# 修改供应商库，修改为允许下级引用：不指定共公司
def test_update_supplier_pool(json_headers,test_select_save_supplier_pool):
	_data = requestBody.getParamsBody("MES_UPDATE_SUPPLIER_POOL")
	_data['id'] = test_select_save_supplier_pool['data']['list'][0]['id']
	_data['allowSubOrgRel'] = 3
	response = requests.post(
...
# 校验更新后，列表数据
def test_select_update_supplier_pool(json_headers):
	response = requests.post(
		url=requestBody.getUrl("MES_SELECT_SUPPLIER_POOL"),
		headers=json_headers,
		json=requestBody.getParamsBody("MES_SELECT_SUPPLIER_POOL")
	...
	assert response.json()['success'] is True, response.json()['message']
	assert response.json()['data']['list'][0]['allowSubOrgRel'] == 3, "列表允许公司下级引用方式与请求不一致"
	assert response.json()['data']['list'][0]['maintainShopList'][0]['name'] == _companyName, "列表维护公司与请求不一致"
	assert response.json()['data']['list'][0]['relOrganizationList'] == [], "列表引用公司与请求不一致"
	return response.json()
...
# 删除数据
def test_delete_supplier_pool(json_headers, test_select_save_supplier_pool):
	_data = requestBody.getParamsBody("MES_DELETE_SUPPLIER_POOL")
	_data['list'] = [test_select_save_supplier_pool['data']['list'][0]['id']]
	response = requests.post(
...
if __name__ == '__main__':
	pytest.main("-s,-v,test_supplierPool_allowSubOrgRel1.py")
